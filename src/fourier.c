...
// author: vishalpaudel
// 2024-04-06
// Note: For sampling a given `Sketch` (an array of `Vector2` vertices) as a
// continuous function
...
bool updateFourier(struct Cycloid *cycloid, struct Sketch *sketch) {
  for (int i = 0; i < cycloid->numCycles; i++) {
    ...
    if (a_k.x < 0)
      theta = PI + theta; // atan() range of [-pi/2, pi/2], but for Quadrant 2
                          // or Quadrant 3
...
Vector2 calculate_a(struct Sketch *sketch, int k) {
  ...
  for (int m = 0; m < MAX_TIME_POINTS; m++) {
    ...
    float complex complex_exponent =
        e_to_the_j_exponent.x + e_to_the_j_exponent.y * I;
    float complex multiplied = x_t * complex_exponent * DELTA_TIME;
    Vector2 element =
        (Vector2){(float)creal(multiplied), (float)cimag(multiplied)};
...
Vector2 sampleSketch(struct Sketch *sketch, double t) {
  ...
  // assumption, sampling of mouse location in the code will be in line with
  // uniform time
  if (t < 0)
    printf("t less than 0 received %f\n", t);
  if (t > 1)
    printf("t greater than 0 received %f\n", t);
  ...
  if (ceil_index == 0)
    return sketch->vertices[0];
  else if (floor_index == ceil_index)
    return sketch->vertices[floor_index];
    ...
    if (ceil_index == sketch->index + 1)
      ceil_index = 0;
    Vector2 floor_point = sketch->vertices[floor_index];
    Vector2 ceil_point = sketch->vertices[ceil_index];

    Vector2 scaled_floor_point =
        Vector2Scale(floor_point, (float)ceil_difference);
    Vector2 scaled_ceil_point =
        Vector2Scale(ceil_point, (float)floor_difference);

    Vector2 linear_interpolated_point;
    linear_interpolated_point =
        Vector2Add(scaled_floor_point, scaled_ceil_point);
