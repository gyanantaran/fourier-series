...
struct Sketch readPointsFile(const char *filename) {
    ...
    float left = INFINITY, right = -1 * INFINITY, top = INFINITY, bottom = -1 * INFINITY;
    ...
    Vector2 screen_centroid = {0.0f * SCREEN_WIDTH, 0.0f * SCREEN_HEIGHT}; // {0.5f * SCREEN_WIDTH, 0.5f * SCREEN_HEIGHT};  // THIS DEPENDS ON THE CAMERA

    float sketch_height = fabsf(bottom - top), sketch_width = fabsf(right - left);

    Vector2 scale_ratios = {SCREEN_WIDTH / sketch_width, SCREEN_HEIGHT / sketch_height};
    printf("scale_ratios.x: %f; scale_ratios.y %f\n", scale_ratios.x, scale_ratios.y);

    float final_scale;
    if (scale_ratios.x <= scale_ratios.y) final_scale = scale_ratios.x;
    else final_scale = scale_ratios.y;

    for (int i = 0; i < point_sketch.index + 1; i++) {
        Vector2 point = point_sketch.vertices[i];
        point_sketch.vertices[i] = Vector2Add(screen_centroid,
