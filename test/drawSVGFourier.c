...
void handleSketching(struct GameState *state, Camera2D *camera,
                     struct Cycloid *myCycloid, struct Sketch *mySketch,
                     struct Sketch *myCycloidSketch);

void handleZooming(struct GameState *state);

void updateCamera(struct GameState *state, Camera2D *camera,
                  struct Sketch *myCycloidSketch, struct Cycloid *myCycloid,
                  Vector2 offset);
...
int main(void) {
  ...
  Vector2 offset = (Vector2){(float)GetScreenWidth() / 2.0f,
                             (float)GetScreenHeight() / 2.0f};

  Camera2D camera = {
      .target = {0, 0}, .offset = offset, .rotation = 0, .zoom = 1.0f};

  struct GameState gameState = {.sketchingFinished = false,
                                .cameraZoomed = false,
  ...
  while (!WindowShouldClose()) {
    // Update
    handleSketching(
        &gameState, &camera, &myCycloid, &mySketch,
        &myCycloidSketch); // state handling  --  Sketching Input from mouse

    updateCycloid(&myCycloid);
    updateSketch(&(myCycloidSketch),
                 myCycloid.outerPoints[myCycloid.numCycles - 1]);

    //        updateSketch(&(myCycloidSketch2),
    //        myCycloid.outerPoints[myCycloid.numCycles - 1 - 10]);
...
void handleZooming(struct GameState *state) {
  if (IsKeyPressed(KEY_SPACE)) {
    ...
    DrawRectangleV((Vector2){0.09 * GetScreenWidth(), 0.19 * GetScreenHeight()},
                   (Vector2){400, 50}, WHITE);
  }
}
...
void updateCamera(struct GameState *state, Camera2D *camera,
  ...
  if (state->cameraZoomed) {
    ...
    for (int d = 0; d < N; d++) {
      last_few_points[d] =
          myCycloidSketch->vertices[myCycloidSketch->index - d];
    }
    ...
    camera->offset = Vector2Add(
        Vector2Scale(
            Vector2Subtract(
                Vector2Add(offset, Vector2Scale(centerPoint, camera->zoom)),
                camera->offset),
            1 / K),
        camera->offset);
    myCycloid->TIME_PERIOD =
        myCycloid->TIME_PERIOD +
        (AFTER_ZOOM_TIME_PERIOD - myCycloid->TIME_PERIOD) / K;
  } else if (!(state->cameraZoomed)) {
    // transition, messy, but mathematical
    camera->offset = Vector2Add(
        Vector2Scale(Vector2Subtract(offset, camera->offset), 1 / (10 * K)),
        camera->offset);
    camera->zoom = camera->zoom + (NORMAL_ZOOM - camera->zoom) / (5 * K);
    myCycloid->TIME_PERIOD = myCycloid->TIME_PERIOD +
                             (NORMAL_TIME_PERIOD - myCycloid->TIME_PERIOD) / K;
  } else {
...
Vector2 find_median_point(Vector2 *points, int N) {
  ...
  if (N % 2 == 1) {
    median_x = points[N / 2].x;
  } else {
    median_x = 0.5f * (points[(N - 1) / 2].x + points[N / 2].x);
  }

  // Find the median of y-coordinates
  qsort(points, N, sizeof(Vector2), compareY);
  if (N % 2 == 1) {
    median_y = points[N / 2].y;
  } else {
    median_y = 0.5f * (points[(N - 1) / 2].y + points[N / 2].y);
  }
