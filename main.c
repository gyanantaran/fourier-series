...
int main(void) {
  ...
  Vector2 window_offset = (Vector2){(float)GetScreenWidth() / 2.0f,
                                    (float)GetScreenHeight() / 2.0f};
  struct GameState state = {.sketchingFinished = true,
                            .cameraZoomed = false,
                            .showAllHarmonics = false,
                            .eraseEverythingSignal = false};
  struct GameObjects objects = {.signal = readPointsFile(POINTS_FILE_PATH),
  ...
  while (!WindowShouldClose()) {
    Vector2 window_offset = (Vector2){(float)GetScreenWidth() / 2.0f,
                                      (float)GetScreenHeight() / 2.0f};
    objects.camera.offset = window_offset;
    printf("Hello\n");
    handle_events(&state);
...
void update(const struct GameState *state, struct GameObjects *objects) {
  if (!state->sketchingFinished) {
    ...
    updateCycloid(&objects->cycloid);
  }

  if (state->addHarmonicSignal) {
    objects->cycloid.numCycles += 1;
  }
  if (state->removeHarmonicSignal) {
    objects->cycloid.numCycles -= 1;
  }
}
...
void draw(const struct GameState *state, struct GameObjects *objects) {
  ...
  DrawText(TextFormat("Number of Cycloids: %d\n\n\n\nPress E to Erase\n\nPress "
                      ...
                      objects->cycloid.numCycles),
           (int)(0.075 * GetScreenWidth()), (int)(0.075 * GetScreenHeight()),
           20, PEN_COLOR);
  DrawText(
      TextFormat("Fourier Series Project\n\nSignals and Systems\n\nVishal "
                 "Paudel\n\nunder the guidance of \n\nProfessor Vivek Deulkar"),
      (int)(0.75 * GetScreenWidth()), (int)(0.8 * GetScreenHeight()), 20,
      TEXT_COLOR);

  BeginMode2D(objects->camera);
  drawSketch(&objects->signal, PEN_COLOR);
  if (state->sketchingFinished)
    drawCycloid(&objects->cycloid);
  EndMode2D();
  EndDrawing();
}
